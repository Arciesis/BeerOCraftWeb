...
class InitInfusionEventSubscriber implements \Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    ...
    public function computeValues(ViewEvent $event)
    {
        if ($event->getControllerResult() instanceof InitInfusion) {
            ...
            $initInfusion->setWaterTempToAdjunct(
                (0.41 / $initInfusion->getWaterGrainRatio()) * ($initInfusion->getWantedMashTemp() - $initInfusion->getInitGrainTemp(
                    )) + $initInfusion->getWantedMashTemp()
            );
        }
    }
}
